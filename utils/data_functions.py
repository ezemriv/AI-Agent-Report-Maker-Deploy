...
from config import FIGURES_FOLDER, MCC_CODES_PATH, EARNINGS_EXPENSES_PLOT_PATH, EXPENSES_SUMMARY_PLOT_PATH
...
def save_earnings_expenses_plot(df: pd.DataFrame):
    ...
    fig.savefig(EARNINGS_EXPENSES_PLOT_PATH, dpi=300, bbox_inches='tight')
    plt.close(fig)
    print(f"Earnings and expenses plot saved at {EARNINGS_EXPENSES_PLOT_PATH}")
...
def save_expenses_summary_plot(df: pd.DataFrame):
    ...
    # Save the plot with higher DPI for better quality
    fig.savefig(EXPENSES_SUMMARY_PLOT_PATH, dpi=300, bbox_inches='tight')
    plt.close(fig)
    print(f"Expenses summary plot saved at {EXPENSES_SUMMARY_PLOT_PATH}")
...
def earnings_and_expenses(
    ...
    earnings = client_data[client_data['amount'] > 0]['amount'].sum()
    expenses = client_data[client_data['amount'] < 0]['amount'].sum()
...
def expenses_summary(
    ...
    client_data.loc[:, 'amount'] = client_data['amount'].abs()

    summary = client_data.groupby('Expenses Type')['amount'].agg([
